...
//   nav: (scrolled) =>
//     `fixed top-0 left-0 right-0 z-50 bg-white transition-all duration-500 m-4 p-4 ${
...
// Improved styling only — no name changes. Kept all export names intact.
...
export const navbarStyles = {
  ...
  logoContainer: "relative flex items-center",
  logoGradient:
    "absolute -inset-3 rounded-full opacity-0 blur-2xl group-hover:opacity-30 transition-opacity duration-400 bg-gradient-to-r from-[#43C6AC] to-[#F8FFAE]",
  logoImage: "h-8 w-8 rounded-full z-10 flex-shrink-0",
  logoText:
  ...
  desktopNavWrapper: "hidden md:flex items-center space-x-2 md:space-x-4",
...
`,
  ...
  navUnderline: (color) =>
    `absolute bottom-0 left-0 h-0.5 w-full rounded-full bg-gradient-to-r ${color} opacity-90`,

  rightIconsWrapper: "hidden md:flex items-center space-x-4",

  cartWrapper: "relative group flex items-center",
  cartGradient:
    "absolute -inset-2 bg-gradient-to-r from-amber-500 to-orange-600 rounded-full opacity-0 blur-md group-hover:opacity-30 transition-opacity duration-400",
  cartIcon:
    "relative h-5 w-5 text-gray-600 group-hover:text-amber-600 transition-colors duration-300 z-10",
  cartBadge:
    "absolute -top-2 -right-2 flex items-center justify-center w-5 h-5 bg-gradient-to-r from-amber-500 to-orange-600 text-[10px] font-bold text-white rounded-full shadow-sm",

  loginWrapper: "relative group flex items-center",
  loginGradient:
    "absolute -inset-2 bg-gradient-to-r from-emerald-500 to-teal-600 rounded-full opacity-0 blur-md group-hover:opacity-30 transition-opacity duration-400",
  loginIcon:
    "relative h-5 w-5 text-gray-600 group-hover:text-emerald-600 transition-colors duration-300 z-10",

  menuBtn: "relative group p-1 rounded-md",
  menuGradient:
    "absolute -inset-2 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-full opacity-0 blur-md group-hover:opacity-30 transition-opacity duration-400",
  menuIcon: "relative h-6 w-6 text-gray-600 z-10",

  mobileMenu:
    "md:hidden absolute inset-x-4 top-full mt-2 bg-white/95 rounded-lg shadow-xl z-40 overflow-hidden",
  mobileContainer: "max-w-3xl mx-auto px-4 py-3",
  mobileNavItem: (isActive, color) =>
    `flex items-center px-4 py-3 rounded-lg ${
      isActive ? `bg-gradient-to-r ${color}/10` : "hover:bg-gray-100"
    } transition-colors duration-200`,

  // Use a safe base color helper so dynamic color parsing doesn't break
  mobileNavIcon: (isActive, color) => {
    const base = navbarStyles.getBaseColor(color) || 'gray';
    return `h-5 w-5 flex-shrink-0 ${isActive ? `text-${base}-500` : 'text-gray-600'}`;
  },

  mobileNavText: (isActive, color) => {
    const base = navbarStyles.getBaseColor(color) || 'gray';
    return `ml-3 truncate ${isActive ? `text-${base}-600 font-medium` : 'text-gray-700'}`;
  },

  mobileIconRow: "flex justify-between items-center mt-4",
  mobileCartBadge:
    "absolute top-0 right-0 -mt-1 -mr-1 w-5 h-5 flex items-center justify-center text-[10px] font-bold text-white bg-gradient-to-r from-amber-500 to-orange-600 rounded-full shadow-sm",

  // Helper: extracts simple base color name (keeps compatibility with input like 'from-emerald-500')
  getBaseColor: (color) => {
    if (!color || typeof color !== 'string') return 'gray';
    // handle hex gradients or custom classes by falling back
    if (color.includes('#')) return 'emerald';
    // common format: 'from-emerald-500' or 'emerald-500' or 'text-emerald-500'
    const match = color.match(/([a-zA-Z]+)(?:-\d{3})?/);
...
export const container =
  "min-h-screen flex items-center justify-center px-4 pt-20 md:pt-28 pb-12 relative bg-gradient-to-br from-[#43C6AC]/90 to-[#2B5876]/90";
...
export const subHeader = "font-light text-3xl sm:text-4xl md:text-5xl text-gray-800";

export const paragraphText =
  "text-gray-700 text-base md:text-lg lg:text-xl leading-relaxed max-w-2xl";

export const formContainer = "flex gap-4 flex-col sm:flex-row items-stretch";
...
export const homeAboutStyles = {
  ...
  bgBlur1: "absolute top-1/4 right-1/4 w-96 h-96 rounded-full bg-[#43C6AC]/20 blur-[100px]",
  bgBlur2: "absolute bottom-1/4 left-1/4 w-96 h-96 rounded-full bg-[#F8FFAE]/20 blur-[100px]",
  container: "max-w-7xl mx-auto px-4 md:px-6 relative z-10",
  aboutGrid: "grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-20",
  imageWrapper: "relative group",
  imageGlow: "absolute -inset-4 bg-gradient-to-r from-[#43C6AC] to-[#F8FFAE] rounded-2xl blur opacity-0 group-hover:opacity-20 transition-opacity duration-500",
  imageContainer: "relative overflow-hidden rounded-xl border-4 border-[#43C6AC]/20",
...
export const booksPageStyles = {
  ...
  bookCard: "group bg-white/90 backdrop-blur-sm rounded-xl md:rounded-2xl p-4 md:p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1",
  imageWrapper: "relative aspect-square mb-4 md:mb-6 overflow-hidden rounded-lg md:rounded-xl",
  imageStyle: "w-full h-full object-contain transform group-hover:scale-105 transition-transform duration-300",
  title: "text-lg md:text-xl font-semibold text-gray-800 mb-1 md:mb-2",
  author: "text-sm text-gray-500 mb-2 md:mb-3",
  ratingWrapper: "flex items-center gap-1 text-yellow-400 text-sm mb-2 md:mb-3",
  description: "text-sm text-gray-600 mb-4",
  priceCartWrapper: "flex items-center justify-between mt-auto",
  price: "text-base md:text-lg font-bold text-[#1A237E]",
  cartButtons: "flex cursor-pointer items-center gap-2 md:gap-3 px-3 md:px-4 py-2 md:py-2.5 bg-gradient-to-r from-[#1A237E] to-[#43C6AC] text-white rounded-lg md:rounded-xl text-sm md:text-base font-medium hover:shadow-md hover:scale-[1.02] transition-all",
};
